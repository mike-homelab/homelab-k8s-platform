...
4. Parse the error stack trace internally and identify what code caused the failure using the local `reasoning` model via the LiteLLM proxy.
5. Recursively invoke the `/coder` workflow strategy, passing the error logs, to formulate and patch the bug via the MCP server using the local `coder` model.

...
5. **Local LLM Analysis**: 
   - Send the context block to the `coding` model via the LiteLLM proxy at `https://llm.michaelhomelab.work/v1/chat/completions`.
...
// turbo
`run_command`: `curl -s -X POST https://llm.michaelhomelab.work/v1/chat/completions -H "Content-Type: application/json" -H "Authorization: Bearer sk-michael-homelab-llm-proxy" -d '{"model": "coding", "messages": [{"role": "user", "content": "Analyze these logs: [PAGE_LOGS]"}]}'`

...
5. Send an internal "Wake request" via `curl` to the LiteLLM proxy at `https://llm.michaelhomelab.work/v1/chat/completions` with a short dummy prompt for both `coder` and `reasoning`. This forces the backend Ollama models to load into GPU VRAM proactively, preventing NGINX 504 timeouts when sending large context payloads.

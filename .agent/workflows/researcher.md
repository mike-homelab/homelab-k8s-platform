...
5. Send an internal "Wake request" via `curl` to the LiteLLM proxy at `https://llm.michaelhomelab.work/v1/chat/completions` with a short dummy prompt for both `vllm-coder` and `vllm-reasoning`. This forces the backend Ollama models to load into GPU VRAM proactively, preventing NGINX 504 timeouts when sending large context payloads.
...
8. Compile the highest-rated context into a polished Research Report (`artifacts/research_plan.md`) using the `vllm-coder` model via the LiteLLM proxy as the primary reasoning engine for data analysis.
